...
public sealed class BlogAppWebApplicationFactory : WebApplicationFactory<WebAPI.Program>, IAsyncLifetime
{
    private const string TestSecurityKey = "integration-test-security-key-at-least-thirty-two-characters";
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder().WithImage("postgres:16-alpine").WithDatabase("blogapp_integration_tests").WithUsername("postgres").WithPassword("postgres").Build();
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, configuration) => configuration.AddInMemoryCollection(new Dictionary<string, string?>
        {
            ...
            ["Token:Audience"] = "blogapp-integration-tests",
            ["Token:Issuer"] = "blogapp-integration-tests",
        ...
        {
            services.RemoveAll<DbContextOptions<BlogAppDbContext>>();
            services.RemoveAll<IMailService>();
            services.AddDbContext<BlogAppDbContext>(options => options.UseNpgsql(ConnectionString));
