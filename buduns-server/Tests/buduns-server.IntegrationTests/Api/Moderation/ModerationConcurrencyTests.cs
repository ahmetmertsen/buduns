...
{
    public ModerationConcurrencyTests(BudunsWebApplicationFactory factory) : base(factory)
    {
    }
    ...
    public async Task Two_moderators_should_not_claim_the_same_report_concurrently()
    {
        ...
        var storedReport = await Factory.ExecuteScopeAsync(async services => await services.GetRequiredService<BudunsDbContext>().Reports.AsNoTracking().SingleAsync(item => item.Id == report.Id));
